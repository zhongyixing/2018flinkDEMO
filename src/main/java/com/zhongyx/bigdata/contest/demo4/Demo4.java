	...
	public static void main(String[] args) throws Exception {
		...
		DataSet<SendMoney> sendMoneyData=sendMoney.map(new MapFunction<String,SendMoney>() {
			...
			.with(new JoinFunction<SendMoney, DeadMan,SendMoney>() {
				...
				public SendMoney join(SendMoney left, DeadMan right) throws Exception {
					...
					{
						return new SendMoney(idenNo, dept, moneyType, year, gender, sendTime, sendMoney,"deadEX");
					}
					else
					{
						return new SendMoney(idenNo, dept, moneyType, year, gender, sendTime, sendMoney,exType);
					}
				}
		...
		.leftOuterJoin(breakManData)
			...
			.with(new JoinFunction<SendMoney, BreakMan,SendMoney>() {
				...
				public SendMoney join(SendMoney left, BreakMan right) throws Exception {
					...
					{
						return new SendMoney(idenNo, dept, moneyType, year, gender, sendTime, sendMoney,"breakEX");
					}
					else
					{
						return new SendMoney(idenNo, dept, moneyType, year, gender, sendTime, sendMoney,exType);
		...
		.leftOuterJoin(masterManData)
			...
			.with(new JoinFunction<SendMoney, MasterMan,SendMoney>() {
				...
				public SendMoney join(SendMoney left, MasterMan right) throws Exception {
					...
					if(right!=null)
					{
						return new SendMoney(idenNo, dept, moneyType, year, gender, sendTime, sendMoney,"masterEX");
					}
					else
					{
						return new SendMoney(idenNo, dept, moneyType, year, gender, sendTime, sendMoney,exType);
		...
		.with(new JoinFunction<SendMoney, RelationMan,SendMoney>() {
			...
			public SendMoney join(SendMoney left, RelationMan right) throws Exception {
				...
				{
					return new SendMoney(idenNo, dept, moneyType, year, gender, sendTime, sendMoney,"relationEX");
				}
				else
				{
					return new SendMoney(idenNo, dept, moneyType, year, gender, sendTime, sendMoney,exType);
		...
		DataSet<Tuple2<String,Integer>> userMoney=sendMoneyData.map(new MapFunction<SendMoney,Tuple2<String,Double>>() {
			@Override
			public Tuple2<String, Double> map(SendMoney value) throws Exception {
				// TODO Auto-generated method stub
				return new Tuple2<String, Double>(value.getIdenNo(), value.getSendMoney());
			}
		}).groupBy(0)
		.sum(1).
		...
		textData.writeAsText("E:\\资料\\高新区比赛\\2018\\flinkOutput\\demo4").setParallelism(1);
